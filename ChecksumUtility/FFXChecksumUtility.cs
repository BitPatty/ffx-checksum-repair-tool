...
namespace FFX.ChecksumUtility
{
  public static class FFXChecksumUtility
  {
    private const ushort CRC_Poly = 0xFFFF;
    private const ushort CRC_T = 0x001A;
    private const ushort CRC_B = 0x64F4;
    private const ushort CRC_Size = 0x64F8;

    private static readonly ushort[] CRC_Mask = {
    ...
    public static void RepairSaveChecksum(this byte[] arr)
    {
      byte[] cs = CRC16CCITT(arr);
      arr[CRC_B] = arr[CRC_T] = cs[0];
      arr[CRC_B + 1] = arr[CRC_T + 1] = cs[1];
    }

    public static byte[] CRC16CCITT(byte[] arr)
    {
      uint CRC_Hash = 0 ^ CRC_Poly;

      for (ushort i = 0x40; i < CRC_Size; i++)
        CRC_Hash = (CRC_Mask[(((i == CRC_B || i == CRC_B + 1) ? 0 : arr[i]) ^ (CRC_Hash >> 8)) & 0xFF] ^ (CRC_Hash << 8)) & 0xFFFF;

      CRC_Hash ^= CRC_Poly;
      return new byte[] { (byte)(CRC_Hash & 0xFF), (byte)((CRC_Hash >> 8) & 0xFF) };
    }

    public static byte[] GetCurrentChecksum(byte[] arr) => new byte[] { arr[CRC_B], arr[CRC_B + 1] };
  }
}
